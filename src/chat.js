 ...
 * Markdown Support: Bold (Inline Emerald) & ### Headings (Small UI Headers)
 */
...
function cycle() {
    ...
    setTimeout(() => {
        ...
        requestAnimationFrame(() => {
            setTimeout(() => textStage.classList.remove('text-incoming'), 50);
        });
    }, 800);
}
...
const addMessage = (text, sender = 'bot') => {
    ...
    let label = 'INTELLIGENCE', bubbleStyle = 'bg-white border border-slate-100 rounded-[1.5rem] rounded-tl-none text-slate-800 shadow-sm';
...
function animate() {
    ...
    dots.forEach((d, i) => {
        ...
        for(let j=i+1; j<dots.length; j++){
            let dist = Math.sqrt((d.x-dots[j].x)**2 + (d.y-dots[j].y)**2);
            if(dist < 110){
                ctx.beginPath();
                ctx.moveTo(d.x, d.y);
